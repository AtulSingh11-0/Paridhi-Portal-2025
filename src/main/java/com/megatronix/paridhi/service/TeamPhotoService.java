...
public class TeamPhotoService {
	...
	public CategorizedTeamPhotoResponse getTeamPhotos() {
		// log the operation
		LoggingUtil.logOperation(
			...
			null,
			"Fetching team photos"
		);

		// fetch team photos by category from the database
		List<TeamPhoto> teamPhotos = teamPhotoRepository.findAll();
		...
		if (teamPhotos.isEmpty()) {
			LoggingUtil.logError(
				...
				AppConstant.TEAM_PHOTO, 
				null,
				"No team photos found", 
				null
			);
			throw new TeamPhotoNotFoundException("No team photos found");
		}

		CategorizedTeamPhotoResponse categorizedTeamPhotos = getCategorizedTeamPhotos(teamPhotos);
		...
		LoggingUtil.logOperation(
			...
			AppConstant.TEAM_PHOTO, 
			null,
			"Successfully retrieved " + teamPhotos.size() + " team photos"
		);

		// return the categorized team photos response
		return categorizedTeamPhotos;
	}
	...
	public TeamPhotoResponse saveTeamPhoto(Category category, MultipartFile teamPhotoImage, User user) {
		...
		var imageDetails = cloudinaryService.uploadFile(teamPhotoImage, AppConstant.TEAM_PHOTO);
	...
	public TeamPhotoResponse updateTeamPhoto(Long id, Category category, MultipartFile teamPhotoImage, User user) {
		...
		.orElseThrow(() -> {
			LoggingUtil.logError(
				...
				AppConstant.TEAM_PHOTO, 
				user,
				String.format(MessageConstant.ErrorTemplate.NOT_FOUND, AppConstant.TEAM_PHOTO), 
				null
			);
			return new TeamPhotoNotFoundException(String.format(MessageConstant.ErrorTemplate.NOT_FOUND, AppConstant.TEAM_PHOTO));
		});
		...
		LoggingUtil.logOperation(
			...
			AppConstant.TEAM_PHOTO, 
			user,
			"Successfully updated team photo with ID: " + id + " - Category: " + updatedTeamPhoto.getCategory() + " - New image public ID: " + imageDetails.get(AppConstant.PUBLIC_ID)
		);
	...
	public void deleteTeamPhoto(Long id, User user) {
		...
		// find the existing team photo by ID
		var existingTeamPhoto = fetchTeamPhotoById(id, user);
	...
	private CategorizedTeamPhotoResponse getCategorizedTeamPhotos(List<TeamPhoto> teamPhotos) {
		// categorize team photos into members and developers
		var categorizedTeamPhotos = new CategorizedTeamPhotoResponse();
		
		categorizedTeamPhotos
			.setMembers(
				teamPhotos.stream()
					.filter(teamPhoto -> teamPhoto.getCategory() == Category.MEGATRONS)
		...
		categorizedTeamPhotos
			.setDevelopers(
				teamPhotos.stream()
				.filter(teamPhoto -> teamPhoto.getCategory() == Category.DEVELOPERS)
				.map(TeamPhotoResponse::fromTeamPhoto)
				.toList()
			);
	...
	private void checkUserAccess(User user, String operation) {
		...
		if (user == null) {
			LoggingUtil.logSecurity(
				...
				null, 
				AppConstant.FAILED,
				"Authentication required to " + operation + AppConstant.TEAM_PHOTO
			);
		...
		if (user.getRole().equals(Role.ROLE_USER)) {
			LoggingUtil.logSecurity(
				...
				user, 
				AppConstant.FAILED,
				"User lacks permission to " + operation + AppConstant.TEAM_PHOTO
			);
			throw new ForbiddenAccessException(MessageConstant.UserMessage.ACCESS_DENIED);
		}
		...
		LoggingUtil.logSecurity(
			...
			user, 
			AppConstant.SUCCESS,
			"User authorized to " + operation + AppConstant.TEAM_PHOTO
		);
	}

	private TeamPhoto fetchTeamPhotoById(Long id, User user) {
		return teamPhotoRepository.findById(id)
		.orElseThrow(() -> {
			LoggingUtil.logError(
				log,
				MessageConstant.Operation.DELETE, 
				AppConstant.TEAM_PHOTO, 
				user,
				String.format(MessageConstant.ErrorTemplate.NOT_FOUND, AppConstant.TEAM_PHOTO, id), 
				null
			);
			return new TeamPhotoNotFoundException(String.format(MessageConstant.ErrorTemplate.NOT_FOUND, AppConstant.TEAM_PHOTO));
		});
