...
public class TeamService {
  ...
  @Transactional(readOnly = true)
	public List<TeamResponse> getTeamsByUser(String email) {
		...
		var teams = teamRepository.findByGidListContainingWithContacts(userGids);
  ...
  @Transactional(readOnly = true)
	public TeamResponse getTeamByTid(String tid) {
		...
		// find the team
		var team = teamRepository.findByTidWithContacts(tid)
				.orElseThrow(() -> {
  ...
  @Transactional(readOnly = true)
	...
	public TeamResponse registerTeam(TeamRequest request) {
			...
			if (teamRepository.existsByTeamNameAndEvent(request.getTeamName(), event)) {
				...
				throw new TeamRegistrationException(
						String.format("Team name '%s' already exists for event: %s", request.getTeamName(), event.getName()));
			}
			...
			if (teamSize < event.getMinPlayers() || teamSize > event.getMaxPlayers()) {
				log.error("Invalid team size: {}. Required: min={}, max={} for event: {}", teamSize, event.getMinPlayers(),
						event.getMaxPlayers(), event.getName());
				throw new TeamRegistrationException(String.format("Team size '%d' must be between: %d and %d for event: %s",
						teamSize, event.getMinPlayers(), event.getMaxPlayers(), event.getName()));
			}
			...
			for (String gid : request.getGidList()) {
				...
				if (gidAlreadyRegistered) {
					...
					throw new TeamRegistrationException(
							String.format("GID '%s' is already registered for event: %s", gid, event.getName()));
				}
			}
			...
			emailService.sendEventRegistration(teamMemberEmails.toArray(new String[0]), event.getName(), team.getTeamName(),
					team.getTid());
	...
	public TeamResponse updatePosition(String tid, Position position) {
		...
		// only send email if position is not 'NONE' and is not the same as the current
		// position
